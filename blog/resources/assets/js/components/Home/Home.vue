        ...
        <div class="row">
            ...
            <div class="popup-post">
                ...
                <div class="scroll-comments" id="commentbody">
                    <div v-if="exist" class="comments_existance"><p>Нет комментариев</p></div>
                    <div class="comments" v-for="comment in comments">
                        <div class="comment">
                            <div class="comment-header">
                                ...
                                <a href="javascript:void(0)" class="comment_reply" @click="Reply(comment.name, comment.id)">Ответить</a>
                            </div>
                    ...
                    <div class="add-comment">
                        <form>
                            <textarea name="text" v-model="comment_text" rows="3" ref="comment_text" type="text" placeholder="Сообщение" class="text-msgs form-control text-message regular-input"></textarea>
                ...
                <div class="card gedf-card">
                    <div class="card-body">
                        ...
                        <ul class="list"  v-for="user in users">
                            <a  @click="GoToProfile(user.id)" href="javascript:void(0)" class="friend-link"><li class="list-item"><img v-bind:src="'http://localhost:8000/' + user.photo" style="width: 20px; height: 20px;"></img> <p id="friendName">{{ user.name }} </p></li></a>
                        </ul>
                    </div>
                    <router-link href="javascript:void(0)" to="/friends" class="card-link" style="text-align:center;">Показать всех</router-link>
                </div>
                <div class="card gedf-card">
                    <div class="card-body">
                    ...
                    <a v-if="count_friends>10" href="javascript:void(0)" class="card-link" style="text-align:center;">Показать всех</a>
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods:{
            ...
            Reply(user_name, id){
                ...
                this.comment_text += '<a href="/profile/' + user_id + '">' + this.reply_name + '</a> , ';
                this.scrollToEnd();
            },
            Logout(){
                let currentObj = this;
